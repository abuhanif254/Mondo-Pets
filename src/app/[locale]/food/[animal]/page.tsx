...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; animal: string }> }): Promise<Metadata> {
  ...
  const title = `${animalName} Food`;

  return {
    title,
    description: `Premium ${animalName.toLowerCase()} food and diet essentials. ${t('subtitle')}`,
    openGraph: {
      title: `${title} | Mondo Pets`,
      description: `Premium ${animalName.toLowerCase()} food and diet essentials.`,
      url: `https://mondopets.com/${locale}/food/${animal}`,
      siteName: 'Mondo Pets',
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `Mondo Pets Premium ${animalName} Food`,
        }
      ],
      type: 'website',
    },
...
export default async function AnimalFoodPage({ 
  params, 
  searchParams 
}: { 
  ...
  const t = await getTranslations({ locale, namespace: 'FoodPage' });
  const products = await getProducts('food', animal, sort, { minPrice, maxPrice, lifeStage, brand, petType, minRating });
  const categories = await getCategories();
    ...
    <div className="flex flex-col min-h-screen">
        ...
        <div className="container mx-auto max-w-4xl space-y-4 flex flex-col md:flex-row md:justify-between md:items-end">
          <div>
            <h1 className="text-3xl md:text-4xl font-extrabold text-foreground">
              {activeCategory ? `${activeCategory.name} Food` : t('title')}
            </h1>
            <p className="text-lg text-muted-foreground">{t('subtitle')}</p>
          </div>
          
          <div className="flex flex-wrap gap-2 mt-4 md:mt-0">
            <Link href="/food" className="rounded-full bg-secondary px-4 py-1.5 text-xs font-medium text-secondary-foreground hover:bg-secondary/80 transition-colors">
              All
            </Link>
            {categories.map((cat) => (
              <Link 
      ...
      <main className="flex-1 py-12 px-6">
        <div className="container mx-auto flex flex-col md:flex-row gap-8">
          ...
          <SidebarFilters categories={categories} currentCategorySlug={animal} basePath="/food" />
          ...
          <div className="flex-1">
            ...
            {products.length === 0 && (
              <div className="py-12 text-center text-muted-foreground border border-dashed rounded-lg mt-8">
                No food products found for this animal.
              </div>
            )}
          </div>
        </div>
